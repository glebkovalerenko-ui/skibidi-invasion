...
class IntroScreen {
    constructor(ctx, options = {}) {
        ...
        this.titleMusic.loop = true; // Чтобы музыка не кончалась
        this.titleMusic.volume = 1.0; // Громкость на максимум
        this.musicStarted = false;
        // --- FIX AUDIO END ---

        // Создаем функцию для первого запуска
        // --- УСИЛЕННЫЙ ЗАПУСКАТЕЛЬ ---
        const startAudio = () => {
            console.log('User interaction detected. Attempting to start audio...');

            // 1. Будим основной движок звука (AudioContext)
            // Это нужно, чтобы потом работали выстрелы и взрывы
            if (window.game && window.game.audioManager && window.game.audioManager.context) {
                if (window.game.audioManager.context.state === 'suspended') {
                    window.game.audioManager.context.resume().then(() => {
                        console.log('AudioContext resumed!');
                    });
                }
            }

            // 2. Запускаем музыку меню
            if (!this.musicStarted) {
                this.titleMusic.play()
                    .then(() => {
                        this.musicStarted = true;
                        console.log('Menu music playing successfully!');
                        // Удаляем слушатели только если успешно запустили
                        window.removeEventListener('click', startAudio);
                        window.removeEventListener('keydown', startAudio);
                    })
                    .catch(e => {
                        console.error('Music play failed:', e);
                        // Не удаляем слушатели, попробуем на следующем клике
                    });
            }
        };
    ...
    update(delta) {
        /*if (!this.musicStarted) {
            ...
            this.titleMusic.play().catch(e => console.log('Ждем клика для музыки...')); 
            this.musicStarted = true;
        }*/
    ...
    draw() {
        ...
        if (this.isGameOver && this.pressSpaceVisible) {
            ...
            this.ctx.fillText(`${Strings.finalScore}: ${this.finalScore}`, 
                this.virtualWidth / 2, 
                this.virtualHeight * 0.15);  // Moved to top
            this.ctx.fillText(`${Strings.highScore}: ${this.highScore}`, 
                this.virtualWidth / 2, 
            ...
            this.ctx.fillText(Strings.gameOver, 
                this.virtualWidth / 2, 
        ...
        if (this.pressSpaceVisible) {
            ...
            this.ctx.fillStyle = '#b026ff'; // Фиолетовый
            this.ctx.font = '24px "Press Start 2P"';
            this.ctx.textAlign = 'center';
            this.ctx.fillText(Strings.pressStart, 
                this.virtualWidth / 2, 
